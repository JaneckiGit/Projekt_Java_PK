...
public class WatchlistPanel extends VBox {
    ...
    private final javafx.collections.ObservableList<Instrument> filtered =
    ...
    public ListView<Instrument> listView() { return listView; }
    ...
    public void refreshList() {
        listView.refresh();
        if (currentDetail != null) updateDetailPanel(currentDetail);
    }
    ...
    private void addDetailRow(GridPane grid, int row, String label, Label valueLabel, String color) {
        ...
        if (row == 0) grid.getColumnConstraints().addAll(c0, c1);
    }
    ...
    private void updateDetailPanel(Instrument inst) {
        ...
        if (vol >= 1_000_000) detailVol.setText(String.format("%.2fM", vol / 1_000_000));
        else if (vol >= 1_000) detailVol.setText(String.format("%.1fK", vol / 1_000));
        else                   detailVol.setText(String.format("%.2f", vol));
    ...
    private static class InstrumentCell extends ListCell<Instrument> {
        ...
        protected void updateItem(Instrument inst, boolean empty) {
            ...
            if (empty || inst == null) { setGraphic(null); return; }
            ...
            switch (inst.getType()) {
                case STOCK  -> { badge.setText("STK"); badge.getStyleClass().add("badge-stock"); }
                case CFD    -> { badge.setText("CFD"); badge.getStyleClass().add("badge-cfd"); }
                case CRYPTO -> { badge.setText("CRP"); badge.getStyleClass().add("badge-crypto"); }
            }
